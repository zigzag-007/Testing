:: Updated IAS to generate random keys while activating by Zig Zag::
:: Added more customization First Name, last Name, Email  by Zig Zag::
@set iasver=1.0
@setlocal DisableDelayedExpansion
@echo off

:===============================================================================::
:: Prepare console Scripting options ::
:: Check for Basic Priviledges and Restrictions ::
::===============================================================================::
...
::============================================================================
::
::   IDM Activation Script v1.0 (IAS)
::
...
::  Add custom last name in IDM license info, prefer to write it in English in below line after = sign,
set lname=

::  Add custom email in IDM license info, prefer to write it in English in below line after = sign,
set email=
...
:: Parse and strip args, set behavior flags, enable unattended if needed ::
...
::===========================================================================================================================::
:: Check if the script is running from a batch file (.bat) and not a compiled executable (.exe) ::
:: :PowerShellTest: $ExecutionContext.SessionState.LanguageMode :PowerShellTest: ::

if "%~x0"==".bat" (

powershell -command "$f=[io.file]::ReadAllText('%~f0') -split ':PowerShellTest:\s*';iex ($f[1])" 2>&1 | findstr /i "FullLanguage" >nul || (
cls
%eline%
...
:: If running from a batch file, perform a check to verify that PowerShell is working correctly ::
:: This check is skipped when running from a compiled executable ::
...
echo:                                                               
echo:               [1] Activate IDM with Random Key
...
set "reg=HKCU\SOFTWARE\DownloadManager /v Serial /t REG_SZ /d "%License_key%"" & call :_rcont
...
set "reg=HKU\%_sid%\SOFTWARE\DownloadManager /v Serial /t REG_SZ /d "%License_key%"" & call :_rcont
)

echo:
echo Triggering a few downloads to create license related registry keys, please wait...
echo:
